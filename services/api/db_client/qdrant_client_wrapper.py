...
class QdrantClientWrapper(DbClient):
    def __init__(self, host: str, port: int, embedding_provider: BaseEmbeddingProvider, storage: ElasticClient):
        self.host = host
        self.port = port
    ...
    async def query(self, collection_name: str, query: str, limit: int = 5) -> QueryResponse:
        ...
        log = {
            'event': 'Retrieval from vector storage',
            'retrieved_chunks': [
            {
        ...
        logger.info(**log)
        self.es.add(config.elastic_log_index, log)
